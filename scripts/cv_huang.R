...
# libraries
# note some other things required, called explicitly in code using ::
...
ERA_clim = ERA_clim%>%
  ...
  left_join(., ERA_MAT%>%select(site, year, ERA_temp_offset), by = c('site' = 'site', 'year' = 'year'))%>% 
...
# note dropping PPT for now as not a major part of the story just use
...
cv_forcing_chilling_photoperiod_CV = function(phen_data, clim_data, n_groups = 10, prop_drop = .10,
  ...
  if (by == 'site'){
    ...
    # note don't do all combinations of site, it will take forever
  }
  if (by == 'year'){
  ...
  if (choose){ # combinatorials, only use for years otherwise takes too long
  ...
  for (k in 1:n_groups){
    ...
    fa = lme4::lmer(Force5s ~ lat + (1|Species)+(1|site), data = fit_data,
                   control = lme4::lmerControl(optimizer = "Nelder_Mead"))
    fb = lme4::lmer(Force5s ~ Chill_5_5_april_30_scale + (1|Species)+(1|site),
                  data = fit_data, control = lme4::lmerControl(optimizer = "Nelder_Mead"))
...
# difference all rmses from null
...
#report the summaries for chill
...
# Temporal model figs####
# just select subset for main figs
